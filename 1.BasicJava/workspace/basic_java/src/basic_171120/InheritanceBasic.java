		...
		/*
			...
			18.추상클래스(abstract class)
		 	 ...
		 	 19.인터페이스
 				...
 				-디폴트 메서드와 클래스 메서드
 					:여러 인터페이스에서 동일한 디폴트메서드가 존재할 경우
 					 인터페이스를 구현한 쪽에서 오버라이드 해야한다.	
 					 
			 20. 내부클래스(inner class)
			 	-내부클래스란?
			 		: 클래스내에 선언된 클래스이다.
			 		: 서로 쉽게 접근 할 수 있는 외부에서 볼 때 불필요한 클래스를 감춰, 복잡성을 줄일 수 있다.
			 		
			 	-인스턴스 클래스 (instance class)
			 		: 외부 클래스에서 인스턴스 멤버들과 관련된 작업을 한다.
			 	-스태틱클래스(static class)
			 		: 외부클래스에서 클래스 멤버들과 관련된 작업을 한다.
			 		  특히 클래스메서드에 사용 할 목적으로 작성
			 	-지역클래스(local class)
			 		: 외부클래스의 메서드나 초기화 블럭 내에 선언
